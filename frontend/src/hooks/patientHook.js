...
export default function usePatient() {
  ...
  const dashboard = { active: dashActive, setActive: setDashActive, isSidebarOpen, toggleSidebar, goHome };
  ...
  const addPatient = { form: addForm, setForm: setAddForm, saving: addSaving, err: addErr, onSubmit: addOnSubmit, goBack: addGoBack };
  ...
  const updLoad = useCallback(async (id = routePatientId) => {
    if (!id) return;
    setUpdLoading(true);
    setUpdErr("");
    try {
      ...
      setUpdErr(e?.response?.data?.message || e.message || "Failed to load patient");
    } finally {
      setUpdLoading(false);
    }
  }, [routePatientId]);
  ...
  const updOnSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      setUpdErr(e?.response?.data?.message || e.message || "Failed to update patient");
      setUpdSaving(false);
    }
